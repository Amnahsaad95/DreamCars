...
<div class="container mx-auto px-4 py-8 max-w-6xl">
	<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
		<h2 class="text-2xl font-bold text-gray-800 mb-6">{{ __('messages.create_new_advertisement') }}</h2>
		...
		<form wire:submit.prevent="save" class="space-y-6">
			<!-- Name -->
			<div>
				<label for="FullName" class="block text-sm font-medium text-gray-700">{{ __('messages.full_name') }}</label>
			...
			<!-- Image Upload -->
			<div>
				<label class="block text-sm font-medium text-gray-700">{{ __('messages.ad_image') }}</label>
				...
				@if ($image)
					<div class="mt-4">
						<span class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.preview') }}</span>
			...
			<!-- URL -->
			<div>
				<label for="url" class="block text-sm font-medium text-gray-700">{{ __('messages.target_url') }}</label>
